...
BEGIN
  ...
  ELSE
    ...
    -- Opening balances are excluded: they are dated at account start, so they
    -- are always the oldest row and would otherwise absorb every payment made
    -- against a dispatch. They can still be settled by an explicit (manual)
    -- allocation from the client payments screen.
    IF v_remaining_payment > 0 THEN
      FOR v_inv IN
        SELECT id, amount
        FROM invoices
