...
import {
  ...
  clientApproveBalanceConfirmation,
                  ...
                  {confirmations && confirmations.length > 0 ? (
                      ...
                      <TableRow key={conf.id}>
                        ...
                        <TableCell className="font-semibold">
                          {conf.outstanding_amount != null
                            ? formatCurrency(conf.outstanding_amount)
                            : "—"}
                        </TableCell>
                        ...
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-2">
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => setSelectedLetter(conf)}
                            >
                    ...
                    <TableRow>
                      <TableCell colSpan={6} className="text-center py-10 text-muted-foreground">
                        No balance confirmations found.
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
              ...
              <div className="max-w-[700px] mx-auto space-y-6">
                ...
                <div className="flex justify-between items-start">
                  <div className="space-y-1 text-sm">
                    {selectedLetter.ref_no && (
                      <p>
                        <strong>Ref No:</strong> {selectedLetter.ref_no}
                      </p>
                    )}
                  </div>
                  <div className="text-right text-sm">
                    <p>
                      <strong>Date:</strong> {formatDate(selectedLetter.created_at)}
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-1 mt-4">
                  ...
                  <p className="font-semibold">
                    {selectedLetter.client_name || "____________________"}
                  </p>
                  <p className="whitespace-pre-wrap text-sm">
                    {selectedLetter.client_address || "____________________"}
                  </p>
                </div>

                <div className="my-6">
                  <p className="font-bold underline text-center">
                    Subject: Confirmation of Outstanding Balance as on{" "}
                    {formatDate(selectedLetter.block_date)}
                  </p>
                </div>
                ...
                <div className="space-y-4 text-sm">
                  ...
                  <p>
                    ...
                    request your confirmation of the outstanding balance in your account with us,{" "}
                    <strong>Mundra Brothers</strong>, as at{" "}
                    <strong>{formatDate(selectedLetter.block_date)}</strong>.
                  </p>
                  <p>
                    As per our records, the balance outstanding against your account for the period{" "}
                    <strong>{formatDate(selectedLetter.period_from)}</strong> to{" "}
                    <strong>{formatDate(selectedLetter.period_to)}</strong> stands at:
                  </p>
                      ...
                      <tbody>
                        <tr>
                          ...
                          <td className="border border-gray-400 p-2 font-semibold">
                            {selectedLetter.client_name || "________"}
                          </td>
                        </tr>
                        <tr>
                          <td className="border border-gray-400 p-2 font-medium bg-gray-50">
                        ...
                        <tr>
                          ...
                          <td className="border border-gray-400 p-2 font-semibold text-base">
                            {formatCurrency(selectedLetter.outstanding_amount)}
                          </td>
                        </tr>
                      </tbody>
                    </table>
                  ...
                  <p>
                    Kindly confirm the correctness of the above balance by signing and returning the
                    attached acknowledgement slip at the earliest. In case of any discrepancy,
                    please provide your statement of account along with supporting documents so that
                    we may reconcile and update our records accordingly.
                  </p>
                  <p>
                    If we do not receive your response within <strong>15 days</strong> from the date
                    of this letter, we shall assume that the balance as stated above is agreed and
                    accepted by you.
                  </p>
                ...
                <div className="mt-12 pt-6 border-t-2 border-dashed border-gray-400">
                  ...
                  <div className="space-y-3 text-sm">
                    <p>
                      We, <strong>{selectedLetter.client_name || "____________________"}</strong>,
                      hereby confirm that the balance outstanding in our account with{" "}
                      <strong>Mundra Brothers</strong> as on{" "}
                      <strong>{formatDate(selectedLetter.block_date)}</strong> is{" "}
                      <strong>{formatCurrency(selectedLetter.outstanding_amount)}</strong>.
                    </p>
                    <p className="mt-2">
