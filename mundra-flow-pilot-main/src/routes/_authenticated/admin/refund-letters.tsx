      ...
      <div className="max-w-[1600px] mx-auto">
        <header className="mb-6 print:hidden">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-semibold tracking-tight">Balance Confirmation Letters</h1>
              <p className="text-sm text-muted-foreground">
                Generate a formal balance confirmation letter to be signed and returned by the client.
              </p>
            </div>
            <div className="flex items-center gap-3">
              ...
              <Button onClick={handleSendToClient} disabled={sendMutation.isPending} className="gap-2">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <Input value={refNo} onChange={(e) => setRefNo(e.target.value)} placeholder="e.g. MB/BC/2024-25/001" />
                </div>
                <div className="space-y-1">
                  <Label>Client Name</Label>
                  <Select value={clientName} onValueChange={(val) => {
                    setClientName(val);
                    const client = clients?.find((c: any) => (c.trade_name || c.legal_name) === val);
                    if (client && client.billing_address) {
                      const addr = [client.billing_address.street1, client.billing_address.street2, client.billing_address.city, client.billing_address.state, client.billing_address.zip].filter(Boolean).join(", ");
                ...
                <div className="space-y-1">
                  ...
                  <Input type="number" value={outstandingAmount} onChange={(e) => setOutstandingAmount(e.target.value)} placeholder="e.g. 500000" />
                </div>
                ...
                <div className="max-w-[800px] mx-auto space-y-6">
                  ...
                  <div className="flex justify-between items-start">
                    <div className="space-y-1 text-sm">
                      {refNo && <p><strong>Ref No:</strong> {refNo}</p>}
                    </div>
                  ...
                  <div className="space-y-1 mt-4">
                    ...
                    <p className="whitespace-pre-wrap text-sm">{clientAddress || "____________________"}</p>
                  </div>
                  ...
                  <div className="space-y-4 text-sm">
                    <p>Dear Sir/Madam,</p>
                    <p>
                      As part of our routine accounting and audit procedures, we are writing to request your confirmation of the outstanding balance in your account with us, <strong>Mundra Brothers</strong>, as at <strong>{formatDate(asOfDate)}</strong>.
                    </p>
                    <p>
                      As per our records, the balance outstanding against your account for the period <strong>{formatDate(periodFrom)}</strong> to <strong>{formatDate(periodTo)}</strong> stands at:
                    </p>
                    ...
                    <div className="my-4">
                      <table className="w-full max-w-md border-collapse border border-gray-400 text-sm">
                        <tbody>
                          <tr>
                            <td className="border border-gray-400 p-2 font-medium bg-gray-50 print:bg-transparent">Client Name</td>
                            <td className="border border-gray-400 p-2 font-semibold">{clientName || "________"}</td>
                          </tr>
                          <tr>
                            <td className="border border-gray-400 p-2 font-medium bg-gray-50 print:bg-transparent">Balance As On</td>
                            <td className="border border-gray-400 p-2 font-semibold">{formatDate(asOfDate)}</td>
                          </tr>
                          <tr>
                            <td className="border border-gray-400 p-2 font-medium bg-gray-50 print:bg-transparent">Outstanding Amount</td>
                            <td className="border border-gray-400 p-2 font-semibold text-base">{formatCurrency(outstandingAmount)}</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                  ...
                  <div className="mt-12 pt-6 border-t-2 border-dashed border-gray-400">
                    ...
                    <div className="space-y-3 text-sm">
                      <p>
                        We, <strong>{clientName || "____________________"}</strong>, hereby confirm that the balance outstanding in our account with <strong>Mundra Brothers</strong> as on <strong>{formatDate(asOfDate)}</strong> is <strong>{formatCurrency(outstandingAmount)}</strong>.
                      </p>
