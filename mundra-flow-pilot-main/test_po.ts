...
async function check() {
  ...
  if (po && po.length > 0) {
    const { data: pay } = await supabase.from('payments').select('*').eq('purchase_order_id', po[0].id);
    console.log("Payments:", JSON.stringify(pay, null, 2));
    const { data: dr } = await supabase.from('dispatch_requests').select('*').eq('purchase_order_id', po[0].id);
    console.log("Dispatches:", JSON.stringify(dr, null, 2));
  } else {
    // maybe PO number is string matching '7' anywhere
    const { data: po_like } = await supabase.from('purchase_orders').select('*').ilike('po_number', '%7%');
