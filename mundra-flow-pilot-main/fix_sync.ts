...
async function fixSync() {
  ...
  const { data: profiles } = await supabase
    ...
    .select("id, organization_id, credit_limit, wallet_balance");
  ...
  for (const client of clients || []) {
    ...
    if (walletBalance !== expectedWalletBalance && profile) {
      console.log(
        `Fixing Client ${client.legal_name}: changing wallet_balance from ${walletBalance} to ${expectedWalletBalance}`,
      );
